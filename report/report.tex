...
\begin{itemize}
    ...
    \item What is a scheduler and does where does it differentiate between systems and languages?
...
This study is limited in scope to a select group of programming languages, with a primary focus on Python. There are two key reasons for this focus:

First, Python, while popular and versatile, is not optimized for performance compared to compiled languages. As an interpreted language, Python typically executes significantly slower than languages like C or C++. This performance gap makes Python an excellent candidate for examining the potential benefits of multi-core threading, as it stands to gain proportionally more from parallel execution improvements.

Second, by concentrating on Python while making comparative references to C, Java, and C\#, we can establish a clearer understanding of fundamental parallel computing concepts. These languages represent different approaches to multi-threading and showcase various implementation strategies for parallelism and concurrency.

Additionally, this study limits its operating system scope to Windows and Linux environments. These operating systems share common chipset compatibility and represent the majority of systems where multi-core processing is applied in practical scenarios. While MacOS is certainly capable of multi-core processing, its hardware ecosystem is more restricted to Apple's specific chipsets, making Windows and Linux more representative of the broader multi-core computing landscape.

\section{Subroutines and Coroutines}
...
In traditional single-core processing, operations follow a synchronous model where instructions execute sequentially in a predetermined order \parencite[p. 118]{Johnson2015}. When expanding to multiple processing units on the same chipset (a multi-core processor), we can maintain this synchronous approach for consistency or adopt an asynchronous model that allows processing units to operate with greater independence.

In practice, synchronous and asynchronous models are rarely implemented in their pure forms. Even highly parallel systems require synchronization mechanisms to coordinate access to shared resources. For instance, memory access operations must be coordinated when multiple cores need to read from or write to the same memory locations simultaneously.

Two fundamental concepts in understanding execution models are subroutines and coroutines:
